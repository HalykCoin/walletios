...
final class HalykNumberFormatter {
    /// Formatter that preserves full precision.
    static let full = HalykNumberFormatter()

    // Formatter that caps the number of decimal digits to 3.
    static let short: HalykNumberFormatter = {
        let formatter = HalykNumberFormatter()
    ...
    func number(from string: String, units: EthereumUnit = .halyk) -> BigInt? {
    ...
    func string(from number: BigInt) -> String {
        let decimals = Int(log10(Double(EthereumUnit.halyk.rawValue)))
        let stringValue = string(from: number, decimals: decimals)
        return stringValue
    }
