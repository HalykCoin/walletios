...
class WalletCoordinator: Coordinator {
    ...
    func createInstantWallet() {
        navigationController.displayLoading(text: NSLocalizedString("wallet.creating.message", value: "Creating wallet...", comment: ""), animated: false)
...
extension WalletCoordinator: BackupViewControllerDelegate {
    ...
    func didPressLater(account: Account, in viewController: BackupViewController) {
        navigationController.confirm(
            ...
            message: NSLocalizedString("export.watchout.text", value: "If this device is replaced or this app is deleted, neither you nor Trust Wallet can recover your funds without a backup", comment: ""),
            okTitle: NSLocalizedString("export.watchout.iunderstand", value: "I understand", comment: ""),
            okStyle: .destructive
        ) { result in
            switch result {
            case .success:
