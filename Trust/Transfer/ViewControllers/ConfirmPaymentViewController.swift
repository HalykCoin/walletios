...
class ConfirmPaymentViewController: UIViewController {
    ...
    lazy var sendTransactionCoordinator = {
        return SendTransactionCoordinator(session: self.session, keystore: self.keystore)
    }()
    lazy var submitButton: UIButton = {
        let button = Button(size: .large, style: .solidBlue)
        button.translatesAutoresizingMaskIntoConstraints = false
        button.setTitle(self.viewModel.sendButtonText, for: .normal)
        ...
        case .ether:
            return TransactionConfiguration(
                speed: TransactionSpeed.custom(
                    gasPrice: self.gasPrice ?? self.transactionSpeed.gasPrice,
                    gasLimit: 90000
                )
            )
        case .halyk:
...
//        navigationItem.rightBarButtonItem = UIBarButtonItem(barButtonSystemItem: .edit, target: self, action: #selector(edit))
    ...
    private func reloadView() {
        ...
        let items: [UIView] = [
            ...
            TransactionAppearance.item(
                title: viewModel.paymentPurchaseIdTitle,
                subTitle: viewModel.paymentPurchaseIdText
            ),
//            TransactionAppearance.item(
    ...
    @objc func send() {
        ...
        case .halyk, .ether:
            self.sendTransactionCoordinator.send(
                ...
                paymentId: transaction.purchaseId,
                amount: amount,
