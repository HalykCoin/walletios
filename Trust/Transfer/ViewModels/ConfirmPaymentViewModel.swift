...
struct ConfirmPaymentViewModel {
    ...
    let config: Config
    private let fullFormatter = HalykNumberFormatter.full
    ...
    var sendButtonText: String {
        return NSLocalizedString("Generic.Send", value: "Send", comment: "")
    }
    ...
    var paymentPurchaseIdTitle: String {
        return NSLocalizedString("confirmPayment.purchaseId", value: "Purchase Id", comment: "")
    }
    var paymentToText: String {
        return transaction.address.address
    }
    var paymentPurchaseIdText: String {
    ...
    var gasPriceText: String {
        ...
        let amount = fullFormatter.string(from: configuration.speed.gasPrice)
        return  String(
            format: "%@ %@",
            amount,
            unit.name
