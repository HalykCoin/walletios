...
class AccountsViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.configure(viewModel: AccountViewModel(account: account, current: Account(address: Address(address: EtherKeystore.current!))))
        return cell
    }

    override func tableView(_ tableView: UITableView, canEditRowAt indexPath: IndexPath) -> Bool {
        return allowsAccountDeletion && (Account(address: Address(address: EtherKeystore.current!)) != viewModel.accounts[indexPath.row] || viewModel.accounts.count == 1)
    }
