...
struct BalanceViewModel: BalanceBaseViewModel {
    ...
    var amountString: String {
        ...
        return "\(balance.balanceString) \(config.server.symbol)"
    }

    var availableAmountString: String {
        guard let balance = balance else { return "" }
        guard !balance.isZero else { return "0.00 \(config.server.symbol)" }
        return "\(balance.availableBalanceString) \(config.server.symbol)"
    }
    ...
    var attributedAmount: NSAttributedString {
        return NSAttributedString(
            string: amountString,
            attributes: smallLabelAttributes
        )
    }

    var attributedCurrencyAmount: NSAttributedString? {
        return NSAttributedString(
            string: availableAmountString,
