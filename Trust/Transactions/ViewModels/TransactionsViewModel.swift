...
struct TransactionsViewModel {
    ...
    var items: [(date: String, transactions: [TransactionHalyk])] = []
    let config: Config

    init(
        transactions: [TransactionHalyk] = [],
        config: Config = Config()
    ) {
        self.config = config

        var newItems: [String: [TransactionHalyk]] = [:]
    ...
    func item(for row: Int, section: Int) -> TransactionHalyk {
    ...
    func titleForHeader(in section: Int) -> String {
        ...
        if NSCalendar.current.isDateInToday(date) { return NSLocalizedString("transactions.title.today", value: "Today", comment: "") }
        if NSCalendar.current.isDateInYesterday(date) { return NSLocalizedString("transactions.title.yesterday", value: "Yesterday", comment: "") }
        return value
    }
