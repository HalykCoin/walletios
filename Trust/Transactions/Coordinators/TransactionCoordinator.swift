...
//        let accountsBarButtonItem = UIBarButtonItem(image: R.image.accountsSwitch(), landscapeImagePhone: R.image.accountsSwitch(), style: .done, target: self, action: #selector(showAccounts))
        ...
        controller.navigationItem.leftBarButtonItem = UIBarButtonItem(image: R.image.menuIcon(), landscapeImagePhone: R.image.import_options(), style: UIBarButtonItemStyle.done, target: self, action: #selector(showSettings))
...
extension TransactionCoordinator: SettingsCoordinatorDelegate {
    func didUpdate(action: SettingsAction, in coordinator: SettingsCoordinator) {
        ...
        case .exportPrivateKey, .pushNotifications, .switchWalletButtonClicked:
            break
        case .donate(let address):
            ...
            removeCoordinator(coordinator)
        case .switchWallet:
            let newAddress = Address(address: keystore.currentAddress)
            let newAccount = Account(address: newAddress, id: keystore.currentId)
            removeCoordinator(coordinator)
            restart(for: newAccount)
        }
    }
...
extension TransactionCoordinator: TransactionsViewControllerDelegate {
    ...
    func didPressTransaction(transaction: TransactionHalyk, in viewController: TransactionsViewController) {
