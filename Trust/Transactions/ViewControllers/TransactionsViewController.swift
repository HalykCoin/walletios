...
class TransactionsViewController: UIViewController {
    ...
    var titleView: BalanceTitleView?
    ...
    init(
        ...
        self.titleView = BalanceTitleView()
    ...
    func fetchBalance() {
        let request = BalanceRequestHalyk(id: session.account.id)
        Session.send(request) { result in
            switch result {
            case .success(let balance):
                let balanceViewModel = BalanceViewModel(balance: balance, rate: .none, config: self.session.config)
                self.session.balanceViewModel = balanceViewModel
                self.session.balance = balance
                self.titleView?.configure(viewModel: balanceViewModel)
            case .failure:
                break
            }
        }
